//CONDITION: Passage of Time with the notrain block FIRST

//timeline for the main task - only in case pretraining is successful
var main_task_pot_notrain = [];
...
timeline_pot_notrain.push(consent);
...
timeline_pot_notrain.push(preload_notrain);
...
timeline_pot_notrain.push(welcome);
...
timeline_pot_notrain.push(survey_age);
...
timeline_pot_notrain.push(survey);
...
timeline_pot_notrain.push(instructions_general);
...
var stimulus_max = ["video/17s_large_3.mp4", "video/17s_large_4.mp4"];
var stimulus_min = ["video/23s_small_1.mp4", "video/23s_small_4.mp4"];
...
timeline_pot_notrain.push(max_presentation);
...
timeline_pot_notrain.push(max_message)
...
timeline_pot_notrain.push(min_presentation);
...
timeline_pot_notrain.push(min_message)
...
timeline_pot_notrain.push(instructions_pre)
...
timeline_pot_notrain.push(pre_training);
...
main_task_pot_notrain.push(instructions_main);

//Success timeline the timeline for the main task is added here
var success = {
    timeline: main_task_pot_notrain,
    conditional_function: function(){
        var n_correct = jsPsych.data.get().filter({correct: true}).count();
        if (n_correct >= 2){
            return true;
...
timeline_pot_notrain.push(pre_repeat);
...
var stimuli_notrain ={
    ...
    main_task_pot_notrain.push(notrain_block);
...
main_task_pot_notrain.push(message);    
...
main_task_pot_notrain.push(preload_train);
...
main_task_pot_notrain.push(message_after_preload);
...
main_task_pot_notrain.push(train_block);
...
var debrief_1 ={
 ...
 main_task_pot_notrain.push(debrief_1);
...
main_task_pot_notrain.push(debrief_2); 
...
main_task_pot_notrain.push(debrief_3);
