//CONDITION: DURATION with the notrain block FIRST

//timeline for the main task - only in case pretraining is successful
var main_task_dur_notrain = [];
...
timeline_dur_notrain.push(consent);
...
timeline_dur_notrain.push(preload_notrain);
...
timeline_dur_notrain.push(welcome);
...
timeline_dur_notrain.push(survey_age);
...
timeline_dur_notrain.push(survey);

// General instructions (text in the js file) 
var instructions_general ={
    type: "instructions",
    pages: [instructions_general_page1_dur, instructions_general_page2_dur],
    show_clickable_nav: true,
    button_label_next: "Suite",
    button_label_previous: "Précédent",
    post_trial_gap: 500
};
timeline_dur_notrain.push(instructions_general);
...
var stimulus_min = ["video/17s_small_2.mp4", "video/17s_small_3.mp4"];
var stimulus_max = ["video/23s_large_1.mp4", "video/23s_large_2.mp4"];
...
timeline_dur_notrain.push(min_presentation);

var min_message = {
    type: "html-keyboard-response",
    stimulus: "Cette vidéo était <b>COURTE</b>, appuyez sur la «flèche pointant vers la gauche»",
    choices: ['ArrowLeft']
};
timeline_dur_notrain.push(min_message)
...
timeline_dur_notrain.push(max_presentation);

var max_message = {
    type: "html-keyboard-response",
    stimulus: "Cette vidéo était <b>LONGUE</b>, appuyez sur la «flèche pointant vers la droite»",
    choices: ['ArrowRight']
};
timeline_dur_notrain.push(max_message)

//Instructions for the pretraining phase
var instructions_pre ={
    type: "instructions",
    pages: [instructions_pretraining_dur],
    show_clickable_nav: true,
    button_label_next: "Suite",
    button_label_previous: "Précédent",
    post_trial_gap: 500
};
timeline_dur_notrain.push(instructions_pre)

//Pretraining response
var response_min ={
    type: "categorize-html",
    stimulus: "courte (flèche gauche) ou plutôt longue (flèche droite)",
...
var pre_training = {
    timeline: [min_presentation, response_min, max_presentation, response_max],
};
timeline_dur_notrain.push(pre_training);
...
//In case pretraining is successful - it contains success message as well as instructions of the main task
var instructions_main = {
    type: "instructions",
    pages: [instructions_main_page1_dur, instructions_main_page2_dur, instructions_main_page3],
    show_clickable_nav: true,
    button_label_next: "Suite",
    button_label_previous: "Précédent",
    post_trial_gap: 500
};
main_task_dur_notrain.push(instructions_main);

//Success timeline the timeline for the main task is added here
var success = {
    timeline: main_task_dur_notrain,
    conditional_function: function(){
        var n_correct = jsPsych.data.get().filter({correct: true}).count();
        if (n_correct >= 2){
            return true;
...
timeline_dur_notrain.push(pre_repeat);
...
var stimuli_notrain ={
    ...
    type: "html-keyboard-response",
    stimulus: "<b><span style='font-size:50px''>courte (flèche gauche)       longue (flèche droite)</span></b>",
    choices: ['ArrowLeft', 'ArrowRight'],
    trial_duration: 4000,
    ...
    main_task_dur_notrain.push(notrain_block);
...
main_task_dur_notrain.push(message); 
...
main_task_dur_notrain.push(preload_train);
...
main_task_dur_notrain.push(message_after_preload);
...
on_finish: function(data) {
   ...
   previous = jsPsych.data.get().last(2).select('stimulus').values[0] //get the stimuli from previous trial
...
main_task_dur_notrain.push(train_block);
...
var debrief_1 ={
 ...
 main_task_dur_notrain.push(debrief_1);
...
main_task_dur_notrain.push(debrief_2); 


//Debrief - Do they want to participate to another condition? 
var debrief_3 = {
    type:"html-keyboard-response",
    stimulus: "<p>Souhaitez-vous participez au suivi de cette étude en ligne pour un supplément de 7,5 € ?</p> <p>Si c'est le cas, veuillez sauvegarder ce lien:</p>" + link_pot + "<p>Merci de l'effectuer le plus tard dans 3 jours ou le lien s'expirera.</p><p>Appuyez sur la touche ENTER lorsque vous êtes prêt.</p>",
    choices: ['Enter']
};
main_task_dur_notrain.push(debrief_3);
